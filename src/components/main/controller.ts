...
import { CheckedInFilter } from '../main/filter/filter'
...
export function filter(allChecked: CheckedInFilter) {
  ...
  renderedProductCards.forEach((element) => {
    ...
    if (allChecked.categories.length >= 1 && allChecked.brands.length === 0) {
      if (allChecked.categories.includes(elementCategory)) {
        element.classList.remove('hide');
      }
    } else if (allChecked.categories.length === 0 && allChecked.brands.length >= 1) {
      if (allChecked.brands.includes(elementBrand)) {
        element.classList.remove('hide');
      }
    } else if (allChecked.categories.length >= 1 && allChecked.brands.length >= 1) {
      if (allChecked.brands.includes(elementBrand) && allChecked.categories.includes(elementCategory)) {
        element.classList.remove('hide');
      }
    } else if (allChecked.categories.length === 0 && allChecked.brands.length === 0) {
      element.classList.remove('hide');
    } else {
      element.classList.add('hide')
    }
